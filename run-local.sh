...
ant -f build.ant deploy-local -DdoClientOptimize=true -DdefaultDomain=$DOMAIN -DtargetDomain=$DOMAIN && (
    cd deploy-local
    ...
    export RHIZI_DEBUG=1
    PYTHONPATH=. python ./bin/rz-server --config-dir etc --debug
)
