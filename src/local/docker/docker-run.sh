...
(
    ...
    docker run --name rhizi.devel.$PORT -it -p $PORT:8080 --link rhizi.neo4j:n4 -v `pwd`/deploy-local:/home/rhizi rhizi/fedora:0.1 bash -c "sed -ie \"$SED_1\" $CONFIG_FILE; sed -ie \"$SED_2\" $CONFIG_FILE; python2.7 bin/rz_server.py --config-dir etc"
)
